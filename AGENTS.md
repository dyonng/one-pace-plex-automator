...
`old_scripts/sync_cast_list.py` (cast actors) once informed a cast-sync feature that has since
been removed (see Casting) — both scripts remain in-repo as behavioral reference.
...
## Casting (removed — post-mortem)

One Pace has no TMDB/TVDB listing, so Plex assigns it no cast. An earlier feature (`syncCast()`)
tried to copy the original series' show-level cast onto the One Pace show via the Plex API. **It was
removed** because it fundamentally cannot work: an agent-less fan-edit show can only receive **bare,
unlinked actor tags** through the metadata API. Plex silently ignores `actor[i].tag.role`, attaches
no character, no photo, and no person GUID/`tagKey` — person-linking comes only from agent matching,
which fan-edit shows don't have. Worse, those bare tags **collide with the source series' real cast
...
`src/poster-sets.ts` is the registry of collections (SpykerNZ + its alt show design follow
`POSTER_REPO_RAW_BASE`; piratezekk / mizzoufan523 / official are pinned to eltharynd/OnePacerr and use
`Season<NN>.png`). Sets are **not uniform** — `posterSetUrl()` returns null where a set has no art
(mizzoufan523 has no show poster) and a 404 is counted as `missing`, not a failure. `syncPosters`
uploads **every** set per target so Plex's own poster picker lists them all, ordering the preferred
set **last** — a Plex upload becomes the selected poster, so going last is what makes it stick (no
separate select call). Bookkeeping is per `(target, setId)` plus a `posters_selected` map; when the
preference changes, only the preferred set is re-uploaded to move the selection.
...
| `POSTER_SET` | `spykernz` | which poster set Plex selects; all sets are uploaded so Plex's picker lists them (dashboard-editable) |
